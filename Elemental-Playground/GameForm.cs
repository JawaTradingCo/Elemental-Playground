    ...
    public partial class GameForm : Form
    {
        ...
        void Update(Object sender, EventArgs e)
        {
            ...
            if (playMode == CONST.PLAY_MODE_RECORD)
            {
                if (isLeftButtonDown)
                {
                    commands.Add(new SetCommand(sim.step, mousePosition.X / pixelSize + 1,
                        mousePosition.Y / pixelSize + 1, brushSize, brushShape, curBrush));
                }
                else if (isRightButtonDown)
                {
                    brushShape = CONST.BRUSH_SHAPE_CIRCLE;
                    commands.Add(new RemoveCommand(sim.step, mousePosition.X / pixelSize + 1,
                        mousePosition.Y / pixelSize + 1, brushSize));
                }
            }
        ...
        void GetBrushes()
        {
            ...
            string @namespace = "Elemental_Playground.Elements";
        ...
        {
            renderer.Render(e.Graphics, sim, pixelSize, simWidth * pixelSize, simHeight * pixelSize, mousePosition,
